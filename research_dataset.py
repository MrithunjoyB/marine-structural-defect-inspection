...
EXPANDED_SYNTHETIC_TYPES=("thin_crack","pitting_cluster","weld_disturbance","colour_only_anomaly","texture_only_anomaly","normal_texture","specular_highlights","illumination_gradient","border_artifact","blur_noise")
...
def prepare_split(registry:DatasetRegistry,dataset_id,ratios=(.7,.15,.15),seed=42,override_leakage=False,mode="Group-aware stratified",override_composition=False,dry_run=False):
    ...
    elif mode=="Expanded Synthetic Benchmark":
        strata={}
        for key,value in groups.items(): strata.setdefault((value["outcome"],value["category"]),[]).append(key)
        split_names=("train","validation","test")
        for stratum in sorted(strata):
            stratum_keys=sorted(strata[stratum]); random.Random(f"{seed}:{stratum[0]}:{stratum[1]}").shuffle(stratum_keys)
            stratum_total=sum(len(groups[key]["ids"]) for key in stratum_keys)
            targets=dict(zip(split_names,[stratum_total*value for value in ratios])); counts={name:0 for name in split_names}
            for key in stratum_keys:
                split=min(split_names,key=lambda name:((counts[name]+len(groups[key]["ids"]))/max(targets[name],1),counts[name],name))
                allocation[key]=split; counts[split]+=len(groups[key]["ids"])
    else:
        strata={}
        for key,value in groups.items(): strata.setdefault((value["outcome"],value["category"]),[]).append(key)
...
def split_blockers(images:pd.DataFrame,mode="Group-aware stratified"):
    ...
    if mode=="Expanded Synthetic Benchmark":
        missing=[item for item in EXPANDED_SYNTHETIC_TYPES if item not in set(test.anomaly_type)]
        if missing:blockers.append(f"test omits expanded benchmark categories: {missing}")
    return blockers
...
def register_expanded_synthetic_benchmark(registry:DatasetRegistry,dataset_id="synthetic-expanded",version="1.0",seed=42,samples_per_category=50,registration_mode="cancel"):
    """Register deterministic generated images without treating empty clean masks as missing annotations."""
    from expanded_synthetic_benchmark import POSITIVE_CATEGORIES, generate_expanded_cases, validate_generated_cases
    cases,parameters=generate_expanded_cases(seed,samples_per_category)
    validation=validate_generated_cases(cases,parameters,samples_per_category)
    if not validation["passed"]: raise ValueError(f"Expanded generation validation failed: {validation['failures']}")
    datasets=registry.datasets(); exists=not datasets.empty and bool(((datasets.dataset_id==dataset_id)&(datasets.dataset_version==version)).any())
    if exists and registration_mode=="cancel": raise ValueError("Dataset ID and version already exist")
    if exists and registration_mode=="replace":
        with registry.connect() as con: con.execute("DELETE FROM images WHERE dataset_id=? AND dataset_version=?",(dataset_id,version))
    metadata=DatasetMetadata(dataset_id,"Expanded Controlled Synthetic Benchmark",version,"synthetic","StructVision expanded generator","local deterministic generator","StructVision-AI","MIT-compatible generated data",True,True,"Generated by expanded_synthetic_benchmark.py",date.today().isoformat(),"structural surface anomalies and imaging nuisances","verified dataset annotation","binary masks",f"master_seed={seed}; samples_per_category={samples_per_category}")
    registry.register_dataset(metadata,overwrite=registration_mode=="replace")
    raw_dir=registry.root/"raw"/dataset_id; ann_dir=registry.root/"annotations"/dataset_id; raw_dir.mkdir(parents=True,exist_ok=True); ann_dir.mkdir(parents=True,exist_ok=True)
    records=[]
    for name,(image,mask) in cases.items():
        ok,encoded=cv2.imencode(".png",image)
        if not ok: raise ValueError(f"Could not encode {name}")
        data=encoded.tobytes(); sha=hashlib.sha256(data).hexdigest(); stored=f"{sha[:12]}_{name}.png"; (raw_dir/stored).write_bytes(data)
        details=parameters[name]; category=str(details["category"]); annotation_path=""
        if category in POSITIVE_CATEGORIES:
            ok,mask_data=cv2.imencode(".png",mask)
            if not ok or not np.any(mask): raise ValueError(f"Invalid positive mask: {name}")
            target=ann_dir/f"{name}.png"; target.write_bytes(mask_data.tobytes()); annotation_path=str(target)
        image_id=str(uuid5(NAMESPACE_URL,f"structvision:{dataset_id}:{version}:{name}:{sha}"))
        records.append(ImageManifestRecord(image_id,dataset_id,f"{name}.png",stored,sha,image.shape[1],image.shape[0],image.shape[2],"png",len(data),metadata.source_name,metadata.licence,metadata.ground_truth_status,annotation_path,"unassigned","unique","valid",datetime.now().isoformat(timespec="seconds"),"",perceptual_hash(image),"","",version,"","","unique",1,str(details["derived_seed"]),category,json.dumps(details,sort_keys=True),category,str(details["image_outcome"]),str(details["source_group_id"]),str(details["template_group_id"]),str(details["near_duplicate_group_id"])))
    registry.add_images(records)
    hashes=[record.sha256_hash for record in records]
    if len(hashes)!=len(set(hashes)): raise AssertionError("Expanded benchmark contains exact duplicates")
    report=build_validation_report(records,{category:samples_per_category for category in EXPANDED_SYNTHETIC_TYPES})
    _save_validation(registry,dataset_id,records,report)
    report_dir=registry.root/"reports"/dataset_id; (report_dir/"generation_parameters.json").write_text(json.dumps({"master_seed":seed,"cases":parameters},indent=2)); (report_dir/"generation_validation.json").write_text(json.dumps(validation,indent=2))
    return records,report,validation
