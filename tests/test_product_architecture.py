...
class ProductArchitectureTests(unittest.TestCase):
    def test_client_imports_only_public_structvision_interface(self):
        client = ROOT / "apps" / "structvision_demo.py"
    ...
    def test_dependency_direction_and_streamlit_isolation(self):
        ...
        for path in algorithm_files:
            ...
            self.assertNotIn("apps.structvision_demo", source)
            self.assertNotIn("import structvision_demo", source)
            self.assertNotIn("import streamlit", source)
        client_source = (ROOT / "apps" / "structvision_demo.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_no_automatic_install_download_or_paid_service(self):
        ...
        self.assertNotIn("private_storage", sources.lower())
